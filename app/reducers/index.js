...
export function addStudentAxios(studentObj, history) {
  return function thunk(dispatch) {
    return axios
      .post("/api/students", studentObj)
      .then(res => res.data)
      .then(createdStudent => {
...
export function addCampusAxios(campusObj, history) {
  return function thunk(dispatch) {
    return axios
      .post("/api/campuses", campusObj)
      .then(res => res.data)
      .then(createdCampus => {
...
export function deleteCampusFromDb(campusId) {
  return function thunk(dispatch) {
    return axios.delete(`/api/campuses/${campusId}`).then(something => {
...
export function StudentUpdateAxios(updatedStudentObj, studentId) {
  return function thunk(dispatch) {
    return axios
      .put(`/api/students/${studentId}`, updatedStudentObj)
...
export function CampusUpdateAxios(updatedCampusObj, campusId) {
  return function thunk(dispatch) {
    return axios
      .put(`/api/campuses/${campusId}`, updatedCampusObj)
      .then(response => response.data)
      .then(updatedCampus => {
  ...
  switch (action.type) {
    ...
    case ADD_STUDENT:
      return Object.assign({}, state, {
        students: [...state.students, action.studentObj]
      });
    ...
    case DELETE_CAMPUS:
      const arrWithoutCampus = state.campuses.filter(campus => {
    ...
    case UPDATE_STUDENT:
      ...
      return Object.assign({}, state, {
        students: [...arrWithoutUpdatedStudent, action.student]
      });
    ...
    case UPDATE_CAMPUS:
      ...
      return Object.assign({}, state, {
        campuses: [...arrWithoutUpdatedCampus, action.campus]
      });
